...
实际上,仓储用于领域对象在数据库(参阅 [实体](Entities.cn.md))中的操作,通常每个**聚合根**或不同的实体创建对应的仓储.

### 通用仓储

ABP为每个聚合根或实体提供了**默认的通用(泛型)仓储**. 你可以在服务中[注入](Dependency-Injection.cn.md) `IRepository<TEntity, TKey>` 使用标准的**CRUD**操作. 用法示例:
...
> 上面的示例在[实体](Entities.cn.md)与[DTO](Data-Transfer-Objects.cn.md)之间使用了手动映射. 参阅 [对象映射](Object-To-Object-Mapping.cn.md) 了解自动映射的使用方式.
...
自定义存储库强依赖于你使用的数据访问工具. 在此示例中, 我们将使用Entity Framework Core：
...
你可以直接使用数据库访问提供程序 (本例中是 `DbContext` ) 来执行操作. 有关基于[EF Core的自定义仓储]的更多信息，请参阅[EF Core 集成文档](Entity-Framework-Core.cn.md).
